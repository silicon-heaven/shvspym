...
void ServerTreeModel::loadSettings(const shv::chainpack::RpcValue &settings)
{
	for(const auto &rv : settings.asList()) {
		...
		m["password"] = QString::fromStdString(Application::instance()->crypt().decrypt(m.value("password").toString().toStdString()));
		createConnection(m);
	}
}
...
void ServerTreeModel::saveSettings(QSettings &settings) const
{
	...
	for(int i=0; i<root->childCount(); i++) {
		...
		props["password"] = QString::fromStdString(Application::instance()->crypt().encrypt(props.value("password").toString().toStdString(), 30));
		lst << props;
	}
